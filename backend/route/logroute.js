...
try{
    ...
    return res.status(400).json({message: "register error vandrichii !! ",error:error.message});
}});
...
try {
    ...
    if(!userlog){
        return res.status(400).json("email illa ...register panuh");
    }


    const loginpass = await bcrypt.compare(req.body.password,userlog.password);
    if(!loginpass){
        return res.status(400).json("password wronggg !!!!")
    }
